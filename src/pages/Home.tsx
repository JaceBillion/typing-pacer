    ...
    >
      <div className="w-full max-w-4xl flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div className="flex flex-wrap justify-center gap-4">
          <div className="flex flex-wrap justify-center gap-2 bg-slate-ui p-1 rounded-2xl border border-white/5">
            {['Easy', 'Medium', 'Hard', 'Drill'].map((diff) => (
              <button
                key={diff}
                onClick={() => setDifficulty(diff as any)}
                className={`px-3 sm:px-4 py-1.5 rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 ${
                  difficulty === diff 
        ...
        <div className="flex items-center gap-4 bg-slate-ui px-4 py-2 rounded-2xl border border-white/5 w-full md:w-auto justify-center">
